...
export function StepCard({ number, title, accent, children, footer }: StepCardProps) {
  return (
    <div
      className={cn(
...
export function FieldGroup({ children, error }: { children: React.ReactNode; error?: boolean }) {
  return (
...
export function StepBtn({ children, accent, variant = 'next', ...props }: StepBtnProps) {
  if (variant === 'back') {
    return (
      <button
        {...props}
